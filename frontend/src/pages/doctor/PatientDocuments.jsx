...
const PatientDocuments = () => {
  ...
  const fetchPatients = async () => {
    try {
      ...
      addNotification({
        type: 'error',
        title: 'Error',
        message: error.response?.data?.message || 'Failed to load assigned patients.'
      });
    }
  };
  ...
  const getFilteredFolders = () => {
    ...
    return Object.values(groupedByPatient).filter((folder) => {
      ...
      const hasMatchingDocument = folder.documents.some(
        (doc) =>
          doc.originalFileName?.toLowerCase().includes(query) ||
          getDocumentTypeLabel(doc.documentType)?.toLowerCase().includes(query) ||
          doc.diseaseName?.toLowerCase().includes(query) ||
          doc.diseaseCode?.toLowerCase().includes(query)
      );

      return patientName.includes(query) || patientEmail.includes(query) || hasMatchingDocument;
    });
  };
  ...
  const getPatientDocuments = () => {
    ...
    const query = searchQuery.toLowerCase();
    return folder.documents.filter(
      (doc) =>
        doc.originalFileName?.toLowerCase().includes(query) ||
        getDocumentTypeLabel(doc.documentType)?.toLowerCase().includes(query) ||
        doc.diseaseName?.toLowerCase().includes(query) ||
  ...
  const handleUpload = async (e) => {
    ...
    if (!diseaseName.trim()) {
      addNotification({
        type: 'error',
        title: 'Validation Error',
        message: 'Please specify the condition associated with this document.'
      });
      return;
    }
    ...
    try {
      ...
      formData.append('diseaseName', diseaseName.trim());
      if (diseaseCode.trim()) {
        formData.append('diseaseCode', diseaseCode.trim());
      }
      if (diagnosisDate) {
  ...
  const handleDownload = async (documentId, fileName) => {
    try {
      ...
      const response = await api.get(`/doctor/patient-documents/${documentId}/download`, {
        responseType: 'blob'
      });
  ...
  const handleDelete = async (documentId) => {
    ...
    try {
      await doctorService.deletePatientDocument(documentId);
      addNotification({
        type: 'success',
    ...
    <div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div className="flex-1">
          ...
          <p className="text-gray-600 text-sm mb-2">
            View and manage documents for your assigned patients.
          </p>
          ...
          {getFilteredFolders().length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 text-lg">
                {patients.length === 0
                  ? 'No assigned patient folders yet.'
                  : searchQuery.trim()
                  ? 'No patients or documents found'
                  : 'No patient folders found'}
              </p>
              {patients.length === 0 && (
                <p className="text-gray-500 text-sm mt-2">
                  Assigned patients will appear here once appointments are scheduled.
                </p>
              )}
            </div>
          ) : (
              ...
              {getFilteredFolders().map((folder) => {
                ...
                return (
                  <div
                    ...
                    className="bg-gradient-to-br from-white to-green-50 border-2 border-gray-200 rounded-lg p-6 cursor-pointer transition-all duration-300 transform hover:border-[#31694E] hover:shadow-xl hover:scale-105 hover:-translate-y-1 group hover:shadow-[#31694E]/20"
                    style={{
                      boxShadow: 'none'
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.boxShadow =
                        '0 10px 25px -5px rgba(49, 105, 78, 0.2), 0 0 0 1px rgba(49, 105, 78, 0.1)';
                    }}
                    onMouseLeave={(e) => {
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-gray-600 text-sm">
                {getPatientDocuments().length}{' '}
                {getPatientDocuments().length === 1 ? 'document' : 'documents'} in this folder
              </p>
            </div>
            <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {getPatientDocuments().map((doc) => {
                ...
                return (
                  <div
                    key={doc._id}
                    className="border-2 border-gray-200 rounded-lg p-4 hover:border-[#31694E] transition-all duration-300 bg-white hover:shadow-[#31694E]/10 shadow-sm transform hover:-translate-y-1"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex items-center gap-3 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-500 mt-1">{getDocumentTypeLabel(doc.documentType)}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 text-sm text-gray-700 mb-3">
                      <p className="font-semibold text-gray-800">
                        Condition: {doc.diseaseName || 'Not specified'}
                        {doc.diseaseCode ? ` (${doc.diseaseCode})` : ''}
                      </p>
                      {doc.diagnosisDate && (
                        <p className="text-gray-600">
                          Diagnosis Date: {new Date(doc.diagnosisDate).toLocaleDateString()}
                        </p>
                      )}
                      {doc.notes && (
      ...
      {showUploadModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-auto my-10 animate-fadeIn">
              ...
              <div className="space-y-4">
                <div className="patient-search-container">
                  ...
                  {showPatientSearchResults && patientSearchResults.length > 0 && (
                    <div className="mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto z-40">
                  ...
                  {showPatientSearchResults && patientSearchQuery.trim() && patientSearchResults.length === 0 && (
                    <div className="mt-1 w-full bg-white border border-gray-300 rounded-lg shadow-lg p-4 z-40">
                ...
                <div>
                  ...
                  <input
                    ...
                    required
                    placeholder="e.g., COVID-19, Dengue, Influenza"
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#31694E] focus:border-[#31694E] focus:outline-none"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="ICD-10 or internal code"
                      className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#31694E] focus:border-[#31694E] focus:outline-none"
                    />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    placeholder="Add context or important observations..."
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#31694E] focus:border-[#31694E] focus:outline-none"
                  />
                </div>
